...
const lootList = [
    "Vault Diamonds", "Larimar", "Carbon", "Knowledge Shards"
] as const;
...
export default function Home() {
    ...
    const [vaultMessage, setVaultMessage] = React.useState(`Use a Vault Rock to Run a vault!`);
    ...
    const [vaultRocks, setVaultRocks] = React.useState(inventory["Vault Rocks"] || 0);
    ...
    const runVault = () => {
        if(!runningVault && vaultRocks >= numRocksForVault){
            ...
            setInventory({...inventory, "Vault Rocks": vaultRocks - numRocksForVault});
...
*/
    ...
    const [vaultDiamonds, setVaultDiamonds] = React.useState(inventory["Vault Diamonds"] || 0);
    const [larimar, setLarimar] = React.useState(inventory["Larimar"] || 0);
    const [carbon, setCarbon] = React.useState(inventory["Carbon"] || 0);
    const [knowledgeShards, setKnowledgeShards] = React.useState(inventory["Knowledge Shards"] || 0);
    const [lastVaultMsg, setLastVaultMsg] = React.useState("No Vault Has Been Run Yet");
    ...
    return (
      <main>
        ...
        <button disabled={runningVault || vaultRocks < numRocksForVault} onClick={runVault}>{vaultMessage}</button>
